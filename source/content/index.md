---
title: Home
---
# Welcome to Brendon's Game Wiki

This is a living reference for the tabletop games I run with friends — a place to keep lore, rules, characters, and campaign notes organised and accessible.

Whether you're a player looking up a character, refreshing your memory on a plot thread, or just curious about the worlds we've built together — you're in the right place.

---

## What's in the Wiki?

> [!info] Work in progress
> This wiki grows alongside our games. Some sections may be sparse while campaigns are just getting started.

### 🎲 [[Dungeons & Dragons]]
Notes, lore, and resources for ongoing D&D campaigns. Includes NPC statblocks, world-building details, and session references.

### ⚔️ [[Age Of Fantasy Skirmish]]
Warbands, rules references, and campaign tracking for our Age of Fantasy Skirmish games. Includes faction notes, rosters, and battle reports.

---

## How to Use This Wiki

- **Players** — Use this as your go-to reference between sessions. Look up NPCs, locations, rules clarifications, and story recaps.
- **Browsing** — Use the search bar or the graph view to explore connections between people, places, and events.
- **Links** — Most entries are cross-linked, so following your curiosity is a great way to explore.

---

*Built with [Obsidian](https://obsidian.md) · Published via [Quartz 4.0](https://quartz.jzhao.xyz) · Maintained by Brendon Duncan*
